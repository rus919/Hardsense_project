...
class NetvarsManager:
    def __init__(self, csgo_handle):
        ...
        client_handle = module_from_name(
            csgo_handle.process_handle, 'client.dll'
        )
        client_bytes = csgo_handle.read_bytes(
        ...
        all_classes = csgo_handle.read_int(client_bytes.find(
        ...
        for table in self._netvars_dict.keys():
            ...
            for prop_name, prop_offset in self._netvars_dict[table].items():
                out_file.write('\t{0:<{1}} 0x{2:08x}\n'.format(
                    prop_name, max_name_len, prop_offset
                ))
