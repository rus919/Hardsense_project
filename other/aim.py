...
class NetVarList:
    def __init__(self):
        ...
        table = NetVarTable('DT_PlantedC4')
        siteplanetd = table.get_offset('m_nBombSite')
        print(siteplanetd)
...
def get_best_target(va, local_p):
    ...
    for i in range(1, Engine.get_max_clients()):
        ...
        if not entity.is_valid():
            continue
        # if u32.GetAsyncKeyState(g_aimbot_key):
        #     if not entity.get_dormant():
        #         continue
        # if u32.GetAsyncKeyState(g_aimbot_key2):
        #     if not entity.get_dormant():
        #         continue
...
if __name__ == "__main__":
    ...
    # threading.Thread(target=trigger2, name='trigger2', daemon=True).start()
    ...
    sens = input('Type your sens: (1 or 1.1): ')
    lower_sens = input('Type your lock sens: (0.2): ')
    # sens_enter = f'sensitivity {sens}\n'
                ...
                try:
                    ...
                    if cross_id != 0 and cross_id < 64:
                        ...
                        if self.get_team_num() != cross_target.get_team_num() and cross_target.get_health() > 0:
                            tn.write(f"sensitivity {lower_sens}\n".encode())
                    ...
                    #         u32.mouse_event(0x0004, 0, 0, 0, 0)
                    #         k32.Sleep(150)
                    if g_aimbot and u32.GetAsyncKeyState(g_aimbot_key):
                        g_aimbot_rcs = True
                        g_aimbot_head = False
                    ...
                    if g_aimbot and u32.GetAsyncKeyState(g_aimbot_key2):
                        g_aimbot_rcs = True
                        g_aimbot_head = True
                        g_aimbot_fov = 3.5 / 180.0
                        g_aimbot_smooth = 1.1
